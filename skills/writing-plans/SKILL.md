...
Steps:
1. Create or update `plans/{slug}.md` in the working directory (single source of truth). Ensure `plans/` exists; derive `{slug}` from the goal (lowercase, hyphen-separated, 3–6 words).
